    ...
    Typeahead.prototype = {
        ...
        lookup: function (event) {
            ...
            if (that.ajax) {
                ...
                if (typeof that.source === 'function') {
                    items = that.grepper(that.source(that.query));
                }else{
        ...
        render: function (items) {
            ...
            items = $(items).map(function (i, item) {
                ...
                var misc='';
                if (item[that.options.category] != null) {
                    misc = ' <span class="muted">'+item[that.options.category]+'</span>';
                }

                i.find('a').html(that.highlighter(item[that.options.display], item)+misc);
                return i[0];
            });
        ...
        select: function () {
            ...
            this.options.itemSelected($selectedItem, $selectedItem.data('data-value'), $selectedItem.text());
            return this.hide();
        },
